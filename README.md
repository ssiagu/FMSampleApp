...
1. 在将 Fmap 数据文件放入 `data` 文件夹下。
2. 修改 `fmap\config\FMPGlobalData.js` 中 `mapOptions.appname` 和 `mapOptions.key` 的值为您在官网注册时获取的应用程序名称和密钥。
3. 如果您要加载指定路径的地图数据文件，可通过配置 `fmap\config\FMPGlobalData.js` 中 `mapOptions.mapServerURL` 和 `mapOptions.mapThemeURL` 来指定地图和主题文件的路径。 默认，会自动从蜂鸟云在线加载数据。
4. 其他配置环境可参考 `fmap\config\FMPGlobalData.js` 文件：
...
1. 运行 `node app`
2. 浏览器中访问：`http://localhost:8000/index.html`